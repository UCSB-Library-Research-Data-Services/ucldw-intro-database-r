...
- Compute the average egg volume for each species
- Do it once using `CSV` files + `dplyr`
- Do it once using a personal database (`duckDB`) + `dbplyr`
- Share some thoughts on similarities and differences
...
OK let's do it! Here are our steps:

1. Compute the egg volume
1. Join the egg table to the nest table
1. Join the resulting table to the species table to add scientific names

<br>

### Compute the average of the volume of an egg. We can use the following formula:
...
### Join this information to the nest table, and average by species

```{r}
species_egg_volume_avg <- left_join(eggs_volume_df, nests_csv) %>%
  group_by(Species) %>%
...
### Bring the scientific names

To improve the table readability, we can bring the full scientific names from the species table

```{r}
species_egg_volume_avg %>%
  left_join(species_study, by= join_by(Species==Code))

```

**OK, that were a lot of steps where data types, NAs, bad entries could have been in the way of a successful analysis.** 
...
**It means the full query is going to be ran and save in your R environment. This might slow things down, so you generally want to collect on the smallest data frame you can.**
...
species_egg_vol_avg_db <- left_join(nests_db, eggs_volume_db, by="Nest_ID") %>%
  group_by(Species) %>%
  summarise(egg_volume_avg = mean(egg_volume, na.rm = TRUE)) %>%
  arrange(desc(egg_volume_avg))

species_egg_vol_avg_db

```

What does this SQL query looks like? Add `show_query()` at the end!
...
Time to collect the data into R

```{r}
# Join the egg and nest tables to compute average
species_egg_vol_avg <- left_join(nests_db, eggs_volume_db, by="Nest_ID") %>%
  group_by(Species) %>%
  summarise(egg_volume_avg = mean(egg_volume, na.rm = TRUE)) %>%
  arrange(desc(egg_volume_avg)) %>%
  collect()
